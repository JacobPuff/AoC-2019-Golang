...
func testPanelBot() {
	_, out, getpanels := panelBotHandlers()
...
func panelBotHandlers() (inputHandler, outputHandler, func() map[Point]Panel) {
	var panelPoints = make(map[Point]Panel)
	var firstOutput = true
	var currentPos = Point{0, 0}
	var dir int64 = 0 //Up 0 Right 1 Down 2 Left 3
	in := func() int64 {
		//fmt.Println(panelPoints[currentPos].clr)
	...
	out := func(output int64) {
		if firstOutput {
			...
			//fmt.Print(panelPoints[currentPos], currentPos)
			//This can be shortened to panel.clr = output
			if output == 1 {
				panel.clr = 1
			} else {
				panel.clr = 0
			...
			firstOutput = true
			// fmt.Print(" ", output, "\n")
			//fmt.Print(" ", panelPoints[currentPos])
			if output == 0 {
				dir--
			} else {
				dir++
			...
			//Golang doesnt have a true modulus function, so this will have to do
			if dir < 0 {
				dir = 3
			}
			if dir > 3 {
...
func day11() {
	...
	inHandle, outHandle, getPanelPoints := panelBotHandlers()
	intComp(panelBotIntCodes, inHandle, outHandle)
	var paintedPanels int64 = 0
	panelPoints := getPanelPoints()
	fmt.Println(len(panelPoints))
	for _, val := range panelPoints {
		if val.traveled {
			paintedPanels++
		}
	}
